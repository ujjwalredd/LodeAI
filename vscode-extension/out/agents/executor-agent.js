...
const terminal_manager_1 = require("../utils/terminal-manager");
...
class ExecutorAgent {
    constructor(panel, projectPath, errorHandler, webviewProvider) {
        ...
        this.terminalManager = new terminal_manager_1.TerminalManager(projectPath);
        ...
        this.initializeMCP();
        logger_1.Logger.info('Executor agent initialized with MCP and AI streaming');
    }
    initializeMCP() {
        ...
        logger_1.Logger.info(`Executor agent registered with MCP as: ${this.agentId}`);
    }
    async handleMCPExecutionRequest(message) {
        try {
            ...
            logger_1.Logger.info('Task execution result shared via MCP');
        }
        catch (error) {
            logger_1.Logger.error('MCP execution request failed:', error);
        ...
        try {
            ...
            if (this.panel) {
                this.panel.webview.postMessage({
                    ...
                    logger_1.Logger.warn(`Failed to send notification: ${err.message}`);
                });
            }
    ...
    async executePlan(plan) {
        ...
        logger_1.Logger.info(`Starting execution of ${plan.tasks.length} tasks`);
        ...
        while (taskQueue.length > 0 && attempts < maxAttempts && !this.disposed) {
            ...
            logger_1.Logger.info(`Executing task: ${task.id} - ${task.description}`);
            const result = await this.executeTask(task);
            if (result.success) {
                ...
                if (currentAttempt < this.maxRetryAttempts) {
                    ...
                    logger_1.Logger.warn(`Task failed, attempting error resolution: ${task.id}`);
                    ...
                    if (resolution.fixed && resolution.retry_command) {
                        ...
                        if (retryResult.success) {
                            ...
                            logger_1.Logger.info(`Task completed after retry: ${task.id}`);
                        }
                        else {
                    ...
                    logger_1.Logger.error(`Max retries exceeded for task: ${task.id}`);
                    return false;
                }
            }
        ...
        if (success) {
            ...
            logger_1.Logger.info(`All ${totalTasks} tasks completed successfully`);
        }
        else {
            ...
            logger_1.Logger.error(`Execution incomplete: ${executedTasks.size}/${totalTasks} tasks completed`);
        }
        return success;
    }
    async executeTask(task) {
        ...
        try {
            ...
            switch (task.type) {
                case 'create_folder':
                    result = await this.createFolder(task);
                    break;
                case 'create_file':
                    result = await this.createFile(task);
                    break;
                case 'run_command':
                    result = await this.runCommand(task);
                    break;
                case 'assessment_question':
                    result = await this.createAssessmentQuestion(task);
                    break;
                case 'download_dataset':
                    result = await this.downloadDataset(task);
                    break;
                case 'setup_environment':
                    result = await this.setupEnvironment(task);
                    break;
                case 'install_dependencies':
                    result = await this.installDependencies(task);
                    break;
                case 'create_virtual_env':
                    result = await this.createVirtualEnvironment(task);
                    break;
                case 'run_validation':
            ...
            logger_1.Logger.error(`Task execution error: ${task.id}`, error);
            return {
                success: false,
    ...
    async createFolder(task) {
        if (!task.path)
            return { success: false, error: 'Missing folder path', task };
    ...
    async createFile(task) {
        if (!task.path)
            return { success: false, error: 'Missing file path', task };
        const fullPath = path.join(this.projectPath, task.path);
        try {
            await fs.promises.mkdir(path.dirname(fullPath), { recursive: true });
            ...
            if (fileName.includes('runner') || fileName.includes('test') || fileName === '.lodeai.json') {
                logger_1.Logger.info(`✅ Step 5: Created file: ${task.path}`);
            }
            return { success: true, task };
        }
        catch (error) {
    ...
    async runCommand(task) {
        if (!task.command)
            return { success: false, error: 'Missing command to run', task };
        try {
    ...
    async createAssessmentQuestion(task) {
        try {
    ...
    async downloadDataset(task) {
        if (!task.path)
            return { success: false, error: 'Missing dataset path', task };
        try {
            ...
            if (datasetResult.success) {
                await this.notify('success', 'ai_dataset_downloaded', `AI-recommended dataset downloaded: ${datasetResult.dataset.name}`);
                return { success: true, task };
            }
            else {
        ...
        try {
            ...
            for (const item of items) {
                ...
                try {
                    ...
                    logger_1.Logger.warn(`Could not clean ${item}: ${err.message}`);
                }
            }
        }
        catch (error) {
            logger_1.Logger.error('Failed to clean working directory', error);
            throw error;
        }
    }
    async setupEnvironment(task) {
        try {
    ...
    async downloadRequiredDatasets(jobDescription, techStack) {
        try {
            ...
            if (datasetResult.success) {
                await this.notify('success', 'ai_dataset_downloaded', `AI-recommended dataset downloaded: ${datasetResult.dataset.name} (${datasetResult.dataset.type})`);
                await this.notify('info', 'dataset_reasoning', `AI reasoning: ${datasetResult.dataset.reasoning}`);
                return { success: true, error: undefined, task: { id: 'ai_dataset_download', type: 'download_dataset', description: 'Download AI-recommended dataset' } };
            }
            else {
                ...
                if (syntheticResult.success) {
                    await this.notify('success', 'synthetic_dataset_created', `AI generated synthetic dataset successfully: ${syntheticResult.dataset.name} (${syntheticResult.rows} rows, ${syntheticResult.features} features)`);
                    return { success: true, error: undefined, task: { id: 'synthetic_dataset', type: 'generate_dataset', description: 'Generate synthetic assessment dataset' } };
                }
                else {
                    return { success: false, error: `Both AI dataset search and synthetic generation failed. AI error: ${datasetResult.error}. Synthetic error: ${syntheticResult.error}`, task: { id: 'dataset_fallback', type: 'generate_dataset', description: 'Generate assessment dataset' } };
                }
            }
        }
        catch (error) {
            return { success: false, error: `Dataset download failed: ${error.message}`, task: { id: 'dataset_download', type: 'download_dataset', description: 'Download assessment datasets' } };
        }
    }
    ...
    async createVirtualEnvironment(task) {
        try {
    ...
    dispose() {
        ...
        logger_1.Logger.info('Executor agent disposed');
    }
}
