...
class MCPService extends events_1.EventEmitter {
    constructor() {
        ...
        logger_1.Logger.info('MCP Service initialized');
    }
    static getInstance() {
    ...
    registerDefaultTools() {
        ...
        this.registerTool({
            ...
            execute: async (params) => {
                try {
                    const { ClaudeClient } = await Promise.resolve().then(() => __importStar(require('./claude-client')));
                    const fs = await Promise.resolve().then(() => __importStar(require('fs')));
...
Respond in JSON format:
{
    ...
    "expected_samples": 150
                    ...
                    try {
                        ...
                        logger_1.Logger.error('Failed to parse AI dataset recommendation:', parseError);
                        return {
                            success: false,
                    ...
                    logger_1.Logger.error('AI dataset search failed:', error);
        ...
        this.registerTool({
            ...
            execute: async (params) => {
                try {
                    ...
                    logger_1.Logger.error('Synthetic data generation failed:', error);
                    return { success: false, error: error.message };
                }
            }
        });
        ...
        this.registerTool({
            ...
            execute: async (params) => {
                try {
                    ...
                    if (techStack.includes('pandas'))
                        requirements.push('pandas>=1.5.0');
                    if (techStack.includes('numpy'))
                        requirements.push('numpy>=1.21.0');
                    if (techStack.includes('sklearn') || techStack.includes('scikit-learn'))
                        requirements.push('scikit-learn>=1.1.0');
                    if (techStack.includes('matplotlib'))
                        requirements.push('matplotlib>=3.5.0');
                    if (techStack.includes('seaborn'))
                        requirements.push('seaborn>=0.11.0');
                    if (techStack.includes('requests'))
                        requirements.push('requests>=2.28.0');
                    if (techStack.includes('flask'))
                        requirements.push('flask>=2.2.0');
                    if (techStack.includes('sqlalchemy'))
                        requirements.push('sqlalchemy>=1.4.0');
...
`;
        ...
        this.registerTool({
            ...
            execute: async (params) => {
                try {
                    const { TerminalManager } = await Promise.resolve().then(() => __importStar(require('./terminal-manager')));
                    const path = await Promise.resolve().then(() => __importStar(require('path')));
                    ...
                    logger_1.Logger.error('Terminal execution failed:', error);
                    return { success: false, error: error.message };
                }
            }
        });
        ...
        this.registerTool({
            ...
            execute: async (params) => {
                try {
                    ...
                    if (techStack.includes('sqlalchemy'))
                        requirements.push('sqlalchemy>=1.4.0');
                    if (techStack.includes('jupyter'))
                        requirements.push('jupyter>=1.0.0');
                    const requirementsPath = path.join(projectPath, 'requirements.txt');
                    fs.writeFileSync(requirementsPath, requirements.join('\n'));
                    // Create setup.py
                    const setupContent = `from setuptools import setup, find_packages
...
`;
                    ...
                    logger_1.Logger.error('Full environment setup failed:', error);
                    return { success: false, error: error.message };
                }
            }
        });
    ...
    registerTool(tool) {
        ...
        logger_1.Logger.info(`MCP Tool registered: ${tool.name}`);
    }
    async executeTool(toolName, params, agentId) {
        ...
        logger_1.Logger.info(`Agent ${agentId} executing tool: ${toolName}`);
        try {
    ...
    async handleMessage(message) {
        logger_1.Logger.info(`Processing MCP message: ${message.type} from ${message.agentId}`);
    ...
    clearHistory() {
        ...
        logger_1.Logger.info('MCP message history cleared');
    }
    /**
     * Helper method to download datasets from URLs
     */
    async downloadDatasetFromUrl(url, targetPath) {
        return new Promise((resolve) => {
            ...
            const request = (url.startsWith('https:') ? https : http).get(url, (response) => {
                if (response.statusCode !== 200) {
                    logger_1.Logger.error(`Failed to download dataset: HTTP ${response.statusCode} for ${url}`);
                ...
                file.on('finish', () => {
                    ...
                    if (totalBytes === 0) {
                        logger_1.Logger.error(`Downloaded file is empty: ${targetPath}`);
                        fs.unlink(targetPath, () => resolve(false));
                        return;
                    }
                    ...
                    if (fs.existsSync(targetPath)) {
                        ...
                        if (stats.size === 0) {
                            logger_1.Logger.error(`Downloaded file has 0 bytes: ${targetPath}`);
                            fs.unlink(targetPath, () => resolve(false));
                            return;
                        }
                        ...
                        const lines = content.split('\n').filter((line) => line.trim().length > 0);
                        if (lines.length < 2) {
                            logger_1.Logger.error(`Downloaded file has insufficient data (only ${lines.length} lines): ${targetPath}`);
                            fs.unlink(targetPath, () => resolve(false));
                            return;
                        }
                        logger_1.Logger.info(`Dataset downloaded successfully: ${targetPath} (${stats.size} bytes, ${lines.length} lines)`);
                        resolve(true);
                    }
                    else {
                        logger_1.Logger.error(`Downloaded file does not exist: ${targetPath}`);
                        resolve(false);
                    }
                });
            });
            request.on('error', (err) => {
                logger_1.Logger.error(`Download error for ${url}: ${err.message}`);
                fs.unlink(targetPath, () => resolve(false));
            });
            request.setTimeout(30000, () => {
                request.destroy();
                logger_1.Logger.error(`Download timeout for ${url}`);
                fs.unlink(targetPath, () => resolve(false));
            });
        });
    }
