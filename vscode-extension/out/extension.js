...
const assessment_orchestrator_1 = require("./orchestrator/assessment-orchestrator");
const logger_1 = require("./utils/logger");
const config_1 = require("./utils/config");
const helper_agent_1 = require("./agents/helper-agent");
const sandbox_manager_1 = require("./utils/sandbox-manager");
// Helper function to make HTTP requests with timeout
function makeHttpRequest(url, data, timeoutMs = 10000) {
...
function activate(context) {
    ...
    logger_1.Logger.info('=== LodeAI Extension Activation Started ===');
    // Initialize config
    config_1.Config.initialize(context);
    logger_1.Logger.info('✅ Step 1.1: Logger initialized');
    logger_1.Logger.info('✅ Step 1.2: Config initialized');
    let currentAssignment = null;
    let mainPanel;
    // Create WebviewViewProvider for sidebar authentication
    class LodeAIWebviewProvider {
        constructor(_extensionUri, _context) {
            this._extensionUri = _extensionUri;
            this._context = _context;
        }
        resolveWebviewView(webviewView, context, _token) {
            logger_1.Logger.info(`resolveWebviewView called for viewType: ${LodeAIWebviewProvider.viewType}`);
            ...
            logger_1.Logger.info(`Webview HTML set successfully, length: ${html.length}`);
                ...
                if (workspaceFolders && workspaceFolders.length > 0) {
                    ...
                    if (fs.existsSync(markerPath)) {
                        try {
                            ...
                            if (markerData.skipAuth) {
                                logger_1.Logger.info('Assessment folder opened in new window - skipping authentication');
                            ...
                            logger_1.Logger.warn('Failed to parse assessment marker:', error);
                        }
                    }
                    ...
                    if (fs.existsSync(metadataPath)) {
                        try {
                            ...
                            logger_1.Logger.info(`Assessment metadata loaded: ${JSON.stringify(assessmentMetadata)}`);
                        }
                        catch (error) {
                            logger_1.Logger.warn('Failed to parse assessment metadata:', error);
                        }
                    }
                    ...
                    if (isAssessment) {
                        ...
                        logger_1.Logger.info('Assessment folder detected, showing test controls');
                        webviewView.webview.postMessage({
                            type: 'assessmentFolderDetected',
                            assessmentPath: folderPath,
            ...
            webviewView.webview.onDidReceiveMessage(async (message) => {
                switch (message.type) {
                    ...
                    case 'getHelperResponse':
                        try {
                            const response = await helper_agent_1.HelperAgent.getResponse(message.message);
                            webviewView.webview.postMessage({
                                type: 'helperResponse',
                                response: response
                            });
                    ...
                    case 'requestEndAssessmentConfirmation':
                        logger_1.Logger.info('Requesting end assessment confirmation');
                        const confirmEndAssessment = await vscode.window.showWarningMessage(message.message, { modal: true }, 'Yes, End Assessment', 'Cancel');
                    ...
                    case 'requestSubmitAssessmentConfirmation':
                        logger_1.Logger.info('Requesting submit assessment confirmation');
                        const confirmSubmitAssessment = await vscode.window.showWarningMessage(message.message, { modal: true }, 'Yes, Submit', 'Cancel');
                    ...
                    case 'submitAssessment':
                        logger_1.Logger.info('✅ Step 6.5: Submit Assessment requested');
                        ...
                        const confirmSubmitFinal = await vscode.window.showWarningMessage('⚠️ Are you ready to submit your assessment?\n\nThis will:\n• Run final tests\n• Submit your code\n• Close this window\n• Delete the assessment folder\n\nMake sure you have saved all your work!', { modal: true }, 'Yes, Submit', 'Cancel');
                        if (confirmSubmitFinal !== 'Yes, Submit') {
                            logger_1.Logger.info('Assessment submission cancelled by user');
                            webviewView.webview.postMessage({
                        ...
                        if (submissionSuccessful) {
                            logger_1.Logger.info('✅ Assessment submitted successfully with passing tests');
                        }
                        else {
                            logger_1.Logger.info('⚠️ Assessment submitted with failing tests');
                        }
                        ...
                        try {
                            logger_1.Logger.info(`Deleting assessment folder: ${assessmentPath2}`);
                            ...
                            logger_1.Logger.info('✅ Assessment folder deleted successfully');
                        }
                        catch (error) {
                            logger_1.Logger.error('Failed to delete assessment folder:', error);
                        }
                        ...
                        break;
                    case 'endAssessment':
                        logger_1.Logger.info('✅ Step 7: End Assessment requested');
                        ...
                        // Show confirmation dialog
                        const confirmEndFinal = await vscode.window.showWarningMessage('⚠️ Are you sure you want to end this assessment?\n\nThis will:\n• Close this window\n• Delete the assessment folder\n• Clear all data\n\nThis action cannot be undone!', { modal: true }, 'Yes, End Assessment', 'Cancel');
                        ...
                        // Delete assessment folder
                        try {
                            logger_1.Logger.info(`Deleting assessment folder: ${assessmentPath3}`);
                            // Use rimraf-style recursive delete
                            const deleteRecursive = (dirPath) => {
                                if (fs.existsSync(dirPath)) {
                                    fs.readdirSync(dirPath).forEach((file) => {
                    ...
                    case 'runTestsInSandbox':
                        ...
                        if (fs.existsSync(metadataPath3)) {
                            try {
                                ...
                                logger_1.Logger.info(`Using main file: ${mainFile}, test command: ${testCommand}`);
                            }
                            catch (error) {
                                logger_1.Logger.warn('Failed to read metadata, using defaults');
                            }
                        }
    ...
    LodeAIWebviewProvider.viewType = 'lodeaiAssistant';
    ...
    logger_1.Logger.info(`WebviewViewProvider registered for viewType: ${LodeAIWebviewProvider.viewType}`);
    // Auto-open the sidebar when extension activates
    setTimeout(() => {
        vscode.commands.executeCommand('lodeai.showSidebar');
    }, 1000);
    async function handleAuthentication(email, webview) {
        logger_1.Logger.info(`Starting authentication for email: ${email}`);
        try {
            // Try new recruiter dashboard system first
            try {
                logger_1.Logger.info('Attempting to connect to recruiter dashboard...');
                logger_1.Logger.info(`Making request to: https://lodeai.vercel.app/api/validate-candidate`);
                logger_1.Logger.info(`Request body: ${JSON.stringify({ email, job_description: '' })}`);
                ...
                logger_1.Logger.info(`API response: ${JSON.stringify(validationResult)}`);
                logger_1.Logger.info(`Valid check: ${validationResult?.valid}, Assignment check: ${!!validationResult?.assignment}`);
                logger_1.Logger.info(`Response type: ${typeof validationResult}`);
                logger_1.Logger.info(`Response keys: ${validationResult ? Object.keys(validationResult) : 'null'}`);
                if (validationResult && validationResult.valid === true && validationResult.assignment) {
                    logger_1.Logger.info('Assignment found, proceeding with authentication...');
                    ...
                    logger_1.Logger.warn('No valid assignment found for email:', email);
                    logger_1.Logger.warn('API response was:', JSON.stringify(validationResult));
                    logger_1.Logger.warn('Response valid field:', validationResult?.valid);
                    logger_1.Logger.warn('Response assignment field:', validationResult?.assignment);
                ...
                logger_1.Logger.error('New validation endpoint failed:', fetchError);
                const errorMessage = fetchError instanceof Error ? fetchError.message : 'Unknown error';
                // Provide more specific error information
                if (errorMessage.includes('Invalid JSON response')) {
                    logger_1.Logger.error('Server returned non-JSON response. This might indicate:');
                    logger_1.Logger.error('1. The endpoint is not available');
                    logger_1.Logger.error('2. The server is returning an HTML error page');
                    logger_1.Logger.error('3. The ngrok tunnel might be down');
                }
                else if (errorMessage.includes('HTTP 4') || errorMessage.includes('HTTP 5')) {
                    logger_1.Logger.error('Server returned HTTP error. Check if the endpoint is working.');
                }
                else if (errorMessage.includes('timeout')) {
                    logger_1.Logger.error('Request timed out. Database connection may be slow or unavailable.');
                }
                else {
                    logger_1.Logger.error('Network or connection error occurred.');
                }
                // No fallback - show clear error message
                logger_1.Logger.error('Authentication failed - no assignment found and no fallback available');
                webview.postMessage({
                    type: 'authError',
    ...
    async function setupAssessment(preferredLanguage) {
        if (!currentAssignment) {
            vscode.window.showErrorMessage('Please authenticate first by entering your email address.');
            return;
        }
        logger_1.Logger.info(`✅ Step 2: User selected language: ${preferredLanguage || 'not specified'}`);
        ...
        try {
            const orchestrator = new assessment_orchestrator_1.AssessmentOrchestrator(undefined, projectPath, webviewProvider);
            ...
            if (success) {
                ...
                const action = await vscode.window.showInformationMessage('✅ Assessment created successfully! Opening in new window...', 'Open Now', 'Stay Here');
                if (action === 'Open Now' || !action) {
                    ...
                    logger_1.Logger.info('✅ Step 4: Opening assessment folder in new window...');
            ...
            logger_1.Logger.error('Assessment setup error:', error);
        }
    }
    ...
    const refreshWebviewCommand = vscode.commands.registerCommand('lodeai.refreshWebview', () => {
        if (webviewProvider._webviewView) {
            ...
            logger_1.Logger.info('Webview refreshed manually');
        }
    });
    // Sandbox commands for running assessments securely
    const sandboxManager = sandbox_manager_1.SandboxManager.getInstance();
    ...
    const runInSandboxCommand = vscode.commands.registerCommand('lodeai.runInSandbox', async () => {
        ...
        if (fs.existsSync(metadataPath)) {
            try {
                ...
                logger_1.Logger.info(`✅ Step 6: Found assessment metadata - Main file: ${mainFile}`);
            }
            catch (error) {
                logger_1.Logger.warn('Failed to parse assessment metadata:', error);
            }
    ...
    logger_1.Logger.info('✅ Step 1.3: Webview provider registered');
    logger_1.Logger.info('✅ Step 1.4: Sandbox manager initialized');
    logger_1.Logger.info('✅ Step 1.5: All commands registered');
    logger_1.Logger.info('=== LodeAI Extension Activation Complete ===');
    logger_1.Logger.info('Extension is ready to use!');
    // Cleanup sandboxes on extension deactivation
    context.subscriptions.push(setupAssessmentCommand, showSidebarCommand, authenticateCommand, refreshWebviewCommand, buildSandboxCommand, runInSandboxCommand, cleanupSandboxCommand, sandboxStatusCommand, webviewView, { dispose: () => sandboxManager.cleanupAllSandboxes() });
}
