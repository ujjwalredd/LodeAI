...
export interface StreamingContext {
    agentType: 'planner' | 'executor' | 'error-handler' | 'orchestrator';
    action: string;
...
export class AIStreamingService {
    ...
    async generateStreamingMessage(context: StreamingContext): Promise<string> {
...
Generate a single, natural sentence or short paragraph that accurately describes what's happening in the given context.`;
        ...
        try {
            ...
            Logger.error('AI streaming message generation failed:', error);
            return this.getFallbackMessage(context);
        }
    }

    private buildContextMessage(context: StreamingContext): string {
    ...
    private getFallbackMessage(context: StreamingContext): string {
    ...
    private getPlannerFallback(context: StreamingContext): string {
    ...
    private getExecutorFallback(context: StreamingContext): string {
        switch (context.action) {
            case 'creating_files':
    ...
    private getErrorHandlerFallback(context: StreamingContext): string {
        switch (context.action) {
    ...
    private getOrchestratorFallback(context: StreamingContext): string {
        switch (context.action) {
            case 'coordinating':
    ...
    async generateStreamingResponse(context: StreamingContext): Promise<AsyncIterable<string>> {
...
Provide a detailed, natural response that explains the current process or status.`;
        ...
        try {
            ...
            Logger.error('AI streaming response generation failed:', error);
            // Return a simple fallback stream
            return this.getFallbackStream(context);
        }
    }

    private async *getFallbackStream(context: StreamingContext): AsyncIterable<string> {
