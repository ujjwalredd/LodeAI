...
import { PlanTask, ErrorResolution } from '../types/assessment-types';
...
export class ErrorHandlerAgent {
    ...
    constructor(panel: vscode.WebviewPanel | undefined, projectPath: string, webviewProvider?: any) {
        this.panel = panel;
        ...
        this.initializeMCP();
        Logger.info('Error handler agent initialized with Claude, MCP and AI streaming');
    }
    ...
    private initializeMCP(): void {
        ...
        Logger.info(`Error handler agent registered with MCP as: ${this.agentId}`);
    }

    private async handleMCPErrorRequest(message: any): Promise<void> {
        try {
            ...
            Logger.info('Error resolution shared via MCP');
        } catch (error: any) {
            Logger.error('MCP error resolution failed:', error);
    ...
    private sendNotification(level: string, content: string) {
        if (this.panel) {
            this.panel.webview.postMessage({ 
                type: 'agentMessage',
                payload: { 
     ...
     */
    async handleError(failedTask: PlanTask, error: string): Promise<ErrorResolution> {
        ...
        if (currentAttempt >= this.maxRetryAttempts) {
            Logger.warn(`Max retries already exceeded for task: ${taskId}, skipping error handling`);
        ...
        Logger.info(`Error handling attempt ${currentAttempt + 1} for task: ${taskId}`);
        ...
        try {
            ...
            if (resolution.fixed) {
                ...
                Logger.info(`Error resolved for task: ${taskId}`);
                ...
                await this.notify('error', 'max_retries_exceeded', failedTask.description);
                Logger.error(`Max retries exceeded for task: ${taskId}. Giving up.`);
            ...
            Logger.error(`Error analysis failed for task ${taskId}`, err);
     ...
     */
    private async analyzeAndFixError(task: PlanTask, error: string, attempt: number): Promise<ErrorResolution> {
     ...
     */
    private async ruleBasedFix(task: PlanTask, error: string, attempt: number): Promise<ErrorResolution> {
     ...
     */
    private fixCommandNotFoundError(task: PlanTask, error: string): ErrorResolution {
    ...
    private fixPermissionError(task: PlanTask, error: string): ErrorResolution {
        let retryCommand = task.command;
    ...
    private fixFileNotFoundError(task: PlanTask, error: string): ErrorResolution {
    ...
    private fixDependencyError(task: PlanTask, error: string): ErrorResolution {
        let retryCommand = task.command;
    ...
    private fixNetworkError(task: PlanTask, error: string, attempt: number): ErrorResolution {
    ...
    private fixSyntaxError(task: PlanTask, error: string): ErrorResolution {
    ...
    private genericFix(task: PlanTask, error: string, attempt: number): ErrorResolution {
     ...
     */
    private async fallbackStrategies(task: PlanTask, error: string): Promise<ErrorResolution> {
        const errorLower = error.toLowerCase();
     ...
     */
    private async analyzeAndFixErrorWithAI(task: PlanTask, error: string, attempt: number): Promise<ErrorResolution> {
        try {
...
Analyze this error and provide a resolution.`;
            ...
            let parsed: ErrorResolution;
            try {
                parsed = JSON.parse(jsonContent);
            } catch (parseErr) {
                Logger.error('Failed to parse Claude response JSON', parseErr);
            ...
            Logger.info(`Claude provided error resolution: ${parsed.fixed ? 'FIXED' : 'NOT_FIXED'}`);
            return parsed;
        } catch (err: any) {
            const message = err?.message || String(err);
