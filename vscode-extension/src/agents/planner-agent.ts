...
export class PlannerAgent {
    ...
    private useAIPlanning: boolean;
    private agentId: string;

    constructor(panel: vscode.WebviewPanel | undefined, webviewProvider?: any) {
        this.panel = panel;
        this.webviewProvider = webviewProvider;
        this.useAIPlanning = Config.getInstance().shouldUseAIPlanning();
        ...
        this.initializeMCP();
        Logger.info('Planner agent initialized with Claude, MCP and AI streaming');
    }
    ...
    private initializeMCP(): void {
        ...
        Logger.info(`Planner agent registered with MCP as: ${this.agentId}`);
    }

    private async handleMCPPlanRequest(message: any): Promise<void> {
        try {
            ...
            Logger.info('Assessment plan shared via MCP');
        } catch (error: any) {
            Logger.error('MCP plan request failed:', error);
            this.mcpService.setSharedData('plan_generation_status', 'failed');
        }
    }

    private async generateAssessmentPlan(jobDescription: string, resumeText: string, assessmentType?: string): Promise<AssessmentPlan> {
    ...
    async createAssessmentPlan(jobDescription: string, resumeText: string, preferredLanguage?: string): Promise<AssessmentPlan> {
        await this.notify('info', 'analyzing_requirements', 'Analyzing candidate profile and job requirements');
        Logger.info(`✅ Step 3: Preferred language received in planner: ${preferredLanguage || 'not specified'}`);

        // DEBUG: Print job description before sending to Claude
        Logger.info('='.repeat(80));
        Logger.info('📋 JOB DESCRIPTION RECEIVED (before Claude API call):');
        Logger.info('='.repeat(80));
        Logger.info(jobDescription);
        Logger.info('='.repeat(80));
        Logger.info(`📊 Job Description Length: ${jobDescription.length} characters`);
        Logger.info(`📝 Resume Length: ${resumeText.length} characters`);
        Logger.info('='.repeat(80));
    ...
    private async createAIPlan(jobDescription: string, resumeText: string, preferredLanguage?: string): Promise<AssessmentPlan> {
...
- Follow the language-specific test file naming and runner templates provided in the instructions above`
        ...
        Logger.info(`🔍 DEBUG - Job Description being sent to Claude (first 300 chars):`);
        Logger.info(jobDescription.substring(0, 300));
        Logger.info(`🔍 DEBUG - Preferred Language: ${preferredLanguage || 'not specified'}`);
...
FOCUS: The assessment MUST match what the job description is asking for. If JD wants algorithm skills, create leetcode_algorithm. If JD wants API skills, create system_design. Read carefully!`;
        ...
        try {
            ...
            Logger.info(`Claude generated assessment plan: ${plan.assessment_type} with ${plan.questions?.length || 0} questions`);
            
            return plan;
        } catch (error: any) {
            Logger.error('Claude planning failed:', error);
            this.mcpService.setSharedData('planning_status', 'failed');
            throw error;
        }
    }

    private async generateDynamicTasks(plan: AssessmentPlan): Promise<PlanTask[]> {
        ...
        const metadata = {
            ...
            main_file: (plan as any).main_file || 'runner.py',
            test_command: (plan as any).test_command || 'pytest -v',
            language: (plan as any).language || 'python',
        ...
        Logger.info(`✅ Step 4: Assessment metadata created:`);
        Logger.info(`  - Language: ${metadata.language}`);
        Logger.info(`  - Main file: ${metadata.main_file}`);
        Logger.info(`  - Test command: ${metadata.test_command}`);
        Logger.info(`  - Assessment type: ${metadata.assessment_type}`);
        ...
        if (plan.project_structure) {
            ...
            rootFiles.forEach((file: string, index: number) => {
                ...
                if (isImageFile) {
                    Logger.warn(`Skipping root image file: ${file} - Only code files are generated`);
        ...
        tasks.push({
            ...
            dependencies: ['create_root']
                ...
                (files as string[]).forEach((file: string, index: number) => {
                    // Skip image and media files
                    const imageExtensions = ['.png', '.jpg', '.jpeg', '.gif', '.svg', '.ico', '.webp', '.bmp', '.tiff'];
                    const isImageFile = imageExtensions.some(ext => file.toLowerCase().endsWith(ext));

                    if (isImageFile) {
                        Logger.warn(`Skipping image file: ${file} - Only code files are generated`);
                        return; // Skip this file
                    }
        ...
        const questionTasks = this.generateQuestionFiles(plan.questions as CodingQuestion[], plan.assessment_type, plan);
        ...
        if (assessmentType === 'leetcode_algorithm') {
            tasks.push({
                ...
                content: this.generateTestFile(plan.questions as CodingQuestion[]),
    ...
    private generateFileContent(filename: string, plan: AssessmentPlan, folderPath?: string): string {
        ...
        switch (filename) {
            ...
            default:
                ...
                if (filename.endsWith('.py')) {
                    ...
                    const question = this.findQuestionForFile(filename, plan.questions as CodingQuestion[]);
    ...
    private generateDynamicReadme(plan: AssessmentPlan): string {
        const questions = plan.questions as CodingQuestion[];
        const assessmentType = plan.assessment_type;
        
        const questionsContent = questions?.map((q, i) => {
            const hints = q.hints ? `\n\n### Hints\n${q.hints.map((h: string) => `- ${h}`).join('\n')}` : '';
...
## Getting Started
${plan.project_structure?.setup_commands?.map((cmd: string) => `1. ${cmd}`).join('\n') || '1. Review the project structure\n2. Implement your solutions\n3. Run tests to verify'}
...
Complete all tasks and ensure your code meets the success criteria.`;
    ...
    private getAssessmentTypeDescription(type: string): string {
        ...
        return descriptions[type as keyof typeof descriptions] || 'Technical skills assessment';
    }

    // FIXED: Only generate datasets for data roles
    private generateDatasetTasks(jobRole: string, techStack: string[]): PlanTask[] {
        const tasks: PlanTask[] = [];
    ...
    private generateQuestionFiles(questions: CodingQuestion[], assessmentType: string, plan?: AssessmentPlan): PlanTask[] {
        const tasks: PlanTask[] = [];

        // Check if we have AI-generated files in the plan
        const generatedFiles = (plan as any)?.generated_files;
    ...
    private generateAIContentFiles(generatedFiles: any, question: CodingQuestion, qNumber: number, assessmentType: string): PlanTask[] {
        const tasks: PlanTask[] = [];
    ...
    private generateDebuggingFiles(question: CodingQuestion, qNumber: number): PlanTask[] {
        const tasks: PlanTask[] = [];
    ...
    private generateBuggyCode(question: CodingQuestion): string {
...
`;
    ...
    private generateDebuggingTests(question: CodingQuestion, qNumber: number): string {
...
if __name__ == "__main__":
    ...
    private generateSystemDesignFiles(question: CodingQuestion, qNumber: number): PlanTask[] {
        const tasks: PlanTask[] = [];
    ...
    private generateAPISpec(question: CodingQuestion, qNumber: number): string {
...
components:
  schemas:
    ...
    private generateDatabaseSchema(question: CodingQuestion, qNumber: number): string {
...
INSERT INTO data_table (value) VALUES 
    ...
    private generateCodeReviewFiles(question: CodingQuestion, qNumber: number): PlanTask[] {
        const tasks: PlanTask[] = [];
    ...
    private generatePoorQualityCode(question: CodingQuestion): string {
        return `#!/usr/bin/env python3
\"\"\"
...
`;
    ...
    private generateReviewGuidelines(question: CodingQuestion, qNumber: number): string {
...
`;
    ...
    private generateImprovedCodeTemplate(question: CodingQuestion): string {
        return `#!/usr/bin/env python3
\"\"\"
...
`;
    ...
    private generateRealWorldProjectFiles(question: CodingQuestion, qNumber: number): PlanTask[] {
        const tasks: PlanTask[] = [];
    ...
    private generateProjectRequirements(question: CodingQuestion): string {
...
`;
    ...
    private generateMainFile(question: CodingQuestion): string {
        return `#!/usr/bin/env python3
\"\"\"
...
`;
    ...
    private generateModelsFile(question: CodingQuestion): string {
        return `#!/usr/bin/env python3
\"\"\"
...
`;
    ...
    private generateProjectTests(question: CodingQuestion): string {
        return `#!/usr/bin/env python3
\"\"\"
...
`;
    ...
    private generateDataAnalysisFiles(question: CodingQuestion, qNumber: number): PlanTask[] {
        const tasks: PlanTask[] = [];
    ...
    private generateAnalysisNotebook(question: CodingQuestion): string {
    ...
    private generateAnalysisScript(question: CodingQuestion): string {
        return `#!/usr/bin/env python3
\"\"\"
...
`;
    ...
    private generateSampleDataset(): string {
...
`;
    ...
    private generateStandardQuestionFiles(question: CodingQuestion, qNumber: number, assessmentType: string): PlanTask[] {
        const tasks: PlanTask[] = [];
    ...
    private generateLeetCodeProblemStatement(question: CodingQuestion, qNumber: number): string {
...
${question.optimal_approach}`;
    ...
    private generateTestFile(questions: CodingQuestion[]): string {
        return `#!/usr/bin/env python3
\"\"\"
...
`;
    ...
    private generateProblemStatement(question: CodingQuestion, qNumber: number): string {
...
${question.constraints?.map(c => `- ${c}`).join('\n')}${hints}`;
    ...
    private generateSolutionTemplate(question: CodingQuestion, qNumber: number): string {
...
if __name__ == "__main__":
    ...
    private generatePythonStarter(filename: string, assessmentType: string, difficulty: string, question?: CodingQuestion): string {
...
if __name__ == "__main__":
    ...
    private generateLeetCodePythonFile(question: CodingQuestion): string {
        const hints = question.hints ? `\n# Hints:\n${question.hints.map(h => `# - ${h}`).join('\n')}` : '';
...
`;
    ...
    private generateSetupPy(plan: AssessmentPlan): string {
        return `#!/usr/bin/env python3
\"\"\"
...
`;
    ...
    private formatPythonValue(inputStr: string): string {
    ...
    private getFunctionCall(signature: string, inputVar: string): string {
        const funcName = signature.split('(')[0].replace('def ', '').trim();
        return `${funcName}(${inputVar})`;
    }

    private findQuestionForFile(filename: string, questions: CodingQuestion[]): CodingQuestion | undefined {
    ...
    private generateJavaScriptStarter(filename: string, assessmentType: string, difficulty: string): string {
        return `// ${filename}
// Assessment Type: ${assessmentType}
// Difficulty: ${difficulty}
...
main();`;
    ...
    private generateNotebookStarter(assessmentType: string, difficulty: string): string {
        return JSON.stringify({
            cells: [
                {
                    cell_type: "markdown",
    ...
    private generateRunnerPy(plan: AssessmentPlan): string {
        const assessmentType = plan.assessment_type;
        const difficulty = plan.difficulty;
        const questions = plan.questions as CodingQuestion[];
...
`;
    ...
    private generatePythonRequirements(jobRole: string, assessmentType?: string): string {
        const base = [
    ...
    private generatePackageJson(jobRole: string, assessmentType?: string): string {
        return JSON.stringify({
            name: "coding-assessment",
            version: "1.0.0",
            type: "module",
    ...
    private async createRuleBasedPlan(jobDescription: string, resumeText: string): Promise<AssessmentPlan> {
        ...
        const plan: AssessmentPlan = {
            assessment_type: this.determineAssessmentType(jobRole, difficulty) as AssessmentType,
            difficulty: difficulty,
            tech_stack: techStack,
            job_role: jobRole,
            requires_datasets: requiresDatasets, // FIXED: Proper flag
        ...
        Logger.info(`Rule-based plan created: ${plan.job_role}, ${plan.questions.length} questions`);

        return plan;
    }

    private determineAssessmentType(jobRole: string, difficulty: DifficultyLevel): string {
        if (difficulty === 'senior') return 'system_design';
        if (jobRole === 'backend' || jobRole === 'fullstack') return 'real_world_project';
        if (difficulty === 'junior') return 'leetcode_algorithm';
        return 'debugging_fixit';
    }

    private determineJobRole(jd: string): string {
        const jdLower = jd.toLowerCase();
        
        if (jdLower.includes('data') && jdLower.includes('scientist')) return 'data_scientist';
        if (jdLower.includes('machine learning') || jdLower.includes('ai')) return 'data_scientist';
        if (jdLower.includes('backend') || jdLower.includes('api') || jdLower.includes('server')) return 'backend';
        if (jdLower.includes('frontend') || jdLower.includes('react') || jdLower.includes('angular')) return 'frontend';
        if (jdLower.includes('full stack') || jdLower.includes('full-stack')) return 'fullstack';
        
        return 'other';
    }

    private generateCodingQuestions(jobRole: string, techStack: string[], difficulty: DifficultyLevel): CodingQuestion[] {
    ...
    private determineDifficulty(jd: string): DifficultyLevel {
        const jdLower = jd.toLowerCase();
        
        if (jdLower.includes('senior') || jdLower.includes('lead') || jdLower.includes('principal')) return 'senior';
        if (jdLower.includes('mid') || jdLower.includes('experienced') || jdLower.includes('3+')) return 'mid';
        return 'junior';
    }

    private extractTechStack(jd: string): string[] {
        const techKeywords = [
    ...
    private extractSkills(jd: string): string[] {
        const skillKeywords = [
    ...
    private analyzeSkillsGap(jd: string, resume: string): { missing: string[]; strong: string[]; recommendations: string[] } {
    ...
    private createFallbackPlan(jobDescription: string): AssessmentPlan {
        Logger.warn('Using fallback coding assessment plan');
