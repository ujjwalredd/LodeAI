...
import { PlannerAgent, ExecutorAgent, ErrorHandlerAgent } from '../agents';
import { AssessmentPlan } from '../types/assessment-types';
...
export class AssessmentOrchestrator {
    ...
    constructor(panel: vscode.WebviewPanel | undefined, projectPath: string, webviewProvider?: any) {
        ...
        this.initializeMCP();
        Logger.info('Assessment orchestrator initialized with MCP coordination and AI streaming');
    }
    ...
    private initializeMCP(): void {
        ...
        this.mcpService.on('agentStateChanged', (data) => {
            Logger.info(`Agent state changed: ${data.agentId}`);
        });

        Logger.info(`Assessment orchestrator registered with MCP`);
    }

    private handleMCPNotification(message: any): void {
    ...
    async createAssessment(jobDescription: string, resumeText: string, preferredLanguage?: string): Promise<boolean> {
        ...
        Logger.info('Starting assessment creation pipeline with MCP');
        Logger.info(`✅ Step 3: Preferred language passed to orchestrator: ${preferredLanguage || 'not specified'}`);
        Logger.info(`📄 Job Description (first 200 chars): ${jobDescription.substring(0, 200)}...`);
        Logger.info(`📄 Resume (first 200 chars): ${resumeText.substring(0, 200)}...`);
        ...
        const errorHandler = new ErrorHandlerAgent(this.panel, this.projectPath, this.webviewProvider);
        const executor = new ExecutorAgent(this.panel, this.projectPath, errorHandler, this.webviewProvider);
        const planner = new PlannerAgent(this.panel, this.webviewProvider);
        ...
        try {
            ...
            Logger.info('Starting planning phase with MCP coordination');
            ...
            Logger.info('Starting execution phase with MCP coordination');
            ...
            if (success) {
                ...
                Logger.info('Starting verification phase with MCP coordination');
                ...
                Logger.info('Assessment creation completed successfully with MCP');
                return true;
            } else {
                ...
                Logger.error('Assessment creation failed during execution phase');
                return false;
            }
            ...
            Logger.info('Assessment orchestrator cleanup completed with MCP');
        }
    }

    private async finalVerification(plan: AssessmentPlan): Promise<void> {
        ...
        const verificationResults: { file: string; exists: boolean; size?: number }[] = [];
        ...
        for (const file of essentialFiles) {
            ...
            if (exists) {
                ...
                Logger.warn(`Verification failed: ${file} not found`);
            }
        }
        ...
        if (plan.assessment_type === 'data_science') {
            ...
            if (dataExists) {
                ...
                Logger.warn('Dataset verification failed');
            }
        }
        ...
        Logger.info(`Verification completed: ${successRate}% success rate`);

        if (successRate >= 80) {
            Logger.info('Assessment verification passed');
        } else {
            Logger.warn('Assessment verification has some issues');
        }
    }
}